-- generator.lua - Test generator for GdUnit4
-- Handles generating test files from parsed C# classes
...
function M.generate_test_method(method_info)
  ...
  local method_call = string.format('_sut.%s(%s)', 
    method_info.name, 
    parser.generate_parameter_values(method_info.parameters))
  ...
  return string.format([[
    ...
    test_name, method_call, method_info.name)
end
...
function M.generate_constructor_setup(file_data)
  ...
  local params = parser.parse_parameters(file_data.constructor_params)
  for _, param in ipairs(params) do
    local param_type, param_name = parser.extract_type_and_name(param)
    if param_type and param_name then
      -- Add field declaration
      table.insert(field_declarations, 
        string.format('    private %s _%s;', param_type, param_name))
      ...
      table.insert(initializations, 
        string.format(init_value, param_name, param_type))
  ...
  return 
    table.concat(field_declarations, '\n'), 
...
function M.generate_test_class(file_data)
  ...
  local field_declarations, initializations, constructor_args = 
    M.generate_constructor_setup(file_data)
